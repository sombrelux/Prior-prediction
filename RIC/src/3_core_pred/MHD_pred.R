...
sig_post <- read.csv('./RIC/src/3_core_pred/MHD_sig.csv',header = T)
parameters <- 'ypred'
...
for(i in 1:4){
  for(k in 1:5){
    data<-list(
      ...
      sig_s = sig_post[7,i])
    samples <- stan(file='./RIC/src/3_core_pred/prior_MHD_normal.stan',
                    data=data,
                    pars=parameters,
                    iter = 1000,
                    warmup = 0,
    ...
    write_csv(prop.1.Option,paste0('./RIC/output/results/core_pred/MHD',k,'_',i,'.csv'))
...
for(i in 1:4){
  prop.1.Option <- NULL
  for(k in 1:5){
    propk <- read_csv(paste0('./RIC/output/results/core_pred/MHD',k,'_',i,'.csv'))
  ...
  write_csv(hdi_eff_mhd,
            paste0('./RIC/output/results/core_pred/hdi_MHD_eff_',i,'.csv'))
}
...
hdi_mhd <- NULL
for(i in 1:4){
  hdi_mhd_i <- read_csv(paste0('./RIC/output/results/core_pred/hdi_MHD_eff_',i,'0.csv'))
  hdi_mhd_i$sigma <- i
