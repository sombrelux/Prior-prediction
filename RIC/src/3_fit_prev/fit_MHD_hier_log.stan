...
parameters{
  ...
  vector<upper=0>[nExp] loghd_i;
  vector<lower=0>[nExp] loghr_i;
  vector<upper=0>[nExp] logsd_i;
  vector<upper=0>[nExp] logsr_i;
  vector<lower=0>[nExp] s_i;
}
transformed parameters{
  vector<lower=0,upper=1>[nExp] hd_i=exp(loghd_i);
  vector<lower=1>[nExp] hr_i=exp(loghr_i);
  vector<lower=0,upper=1>[nExp] sd_i=exp(logsd_i);
  vector<lower=0,upper=1>[nExp] sr_i=exp(logsr_i);
  vector[nTrial] logv1;
  vector[nTrial] logv2;
  vector[nTrial] logw1;
  vector[nTrial] logw2;
...
model{
  ...
  a ~ normal(0,1)T[0,2];
  c ~ normal(0,1)T[0,1];
  logh_r ~ normal(1,1)T[0,];
  logh_d ~ normal(-1,1)T[,0];
  logs_r ~ normal(0,1)T[,0];
  logs_d ~ normal(0,1)T[,0];
  s ~ normal(0,1)T[0,];
  sig ~ normal(0,1)T[0,];
  //individ priors
  for(i in 1:nExp){
      ...
      loghr_i[i] ~ normal(logh_r,sig)T[0,];
      loghd_i[i] ~ normal(logh_d,sig)T[,0];
      logsr_i[i] ~ normal(logs_r,sig)T[,0];
      logsd_i[i] ~ normal(logs_d,sig)T[,0];
