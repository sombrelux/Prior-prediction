...
choice_set <- read_csv("./RIC/data/processed/choice_set.csv")%>%
  filter(choice!='Dom')
post_param <- read_csv('./RIC/output/results/core_pred/MHD_param.csv')
...
for(i in c(1,5,10,100)){
  data<-list(
    ...
    samples <- stan(file='./RIC/src/5_core_pred/prior_MHD_ind.stan',
                data=data,
                pars=parameters,
                iter = 2000,
                warmup = 0,
	...
	saveRDS(samples,paste0('./RIC/output/results/core_pred/prior_MHD_ind_',i,'.rds'))
}
...
for(i in c(1,5,10,100)){
  samples <- readRDS(paste0('./RIC/output/results/core_pred/prior_MHD_ind_',i,'.rds'))
  ...
  write_csv(hdi_mhd,paste0('./RIC/output/results/core_pred/hdi_MHD_ind_',i,'.csv'))
}
