...
for(i in c(3,15,30,300,600,1500)){
  data<-list(
    ...
    half_beta_xo = sig_post[1]*i, half_beta_xa = sig_post[2]*i, half_beta_xr = sig_post[3]*i,
    half_beta_po = sig_post[4]*i, half_beta_pa = sig_post[5]*i, half_beta_pr = sig_post[6]*i,
    half_beta_to = sig_post[7]*i, half_beta_ta = sig_post[8]*i, half_beta_tr = sig_post[9]*i)
  samples <- stan(file='./RIC/src/4_core_pred/prior_RITCH_unif.stan',
                  data=data,
                  pars=parameters,
                  iter = 2000,
                  warmup = 0,
