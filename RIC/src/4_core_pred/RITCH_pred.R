...
choice_set <- read_csv('./RIC/data/processed/choice_set.csv')%>%
  ...
  mutate(xs = sign(xd),ts = sign(td),
         ...
         pr = 2*pd/(p1+p2))
post_param <- read_csv('./RIC/output/results/fit_prev/RITCH_param.csv')
post_param
...
for(i in c(1,5,10,100)){
  data<-list(
    ...
    xr = choice_set$xr, pr = choice_set$pr, tr = choice_set$tr,
  	mu_beta_xo = mu_post[1], mu_beta_xa = mu_post[2], mu_beta_xr = mu_post[3],
  	mu_beta_po = mu_post[4], mu_beta_pa = mu_post[5], mu_beta_pr = mu_post[6],
  	mu_beta_to = mu_post[7], mu_beta_ta = mu_post[8], mu_beta_tr = mu_post[9],
  	sig_beta_xo = sig_post[1]*i, sig_beta_xa = sig_post[2]*i, sig_beta_xr = sig_post[3]*i,
  	sig_beta_po = sig_post[4]*i, sig_beta_pa = sig_post[5]*i, sig_beta_pr = sig_post[6]*i,
  	sig_beta_to = sig_post[7]*i, sig_beta_ta = sig_post[8]*i, sig_beta_tr = sig_post[9]*i)
  samples <- stan(file='./RIC/src/4_core_pred/prior_RITCH_ind.stan',
                data=data,
                pars=parameters,
                iter = 2000,
                warmup = 0,
  ...
  saveRDS(samples,
          paste0('./RIC/output/results/core_pred/prior_RITCH_ind_',i,'.rds'))
}
...
for(i in c(1,5,10,100)){
  samples <- readRDS(paste0('./RIC/output/results/core_pred/prior_RITCH_ind_',i,'.rds'))
  ...
  write_csv(hdi_hd,paste0('./RIC/output/results/core_pred/hdi_RITCH_ind_',i,'.csv'))
}
