...
choice_set <- read_csv("./RIC/data/processed/choice_set.csv")%>%
  filter(choice!='Dom')
post_param <- read_csv('./RIC/output/results/core_pred/PTT_param.csv')
...
for(i in c(1,5,10,100)){
  data<-list(
    ...
    samples <- stan(file='./RIC/src/5_core_pred/prior_PTT_ind.stan',
                data=data,
                pars=parameters,
                iter = 2000,
                warmup = 0,
	...
	saveRDS(samples,paste0('./RIC/output/results/core_pred/prior_PTT_ind_',i,'.rds'))
}
...
for(i in c(1,5,10,100)){
  samples <- readRDS(paste0('./RIC/output/results/core_pred/prior_PTT_ind_',i,'.rds'))
  ...
  write_csv(hdi_hd,paste0('./RIC/output/results/core_pred/hdi_PTT_ind_',i,'.csv'))
}
