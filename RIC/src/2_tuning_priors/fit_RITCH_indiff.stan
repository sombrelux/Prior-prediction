functions{
  real partial_sum(real[] k_slice, int start, int end, int[] Exp, vector xs, vector xd, vector xr, vector ps, vector pd, vector pr, vector ts, vector td, vector tr, vector beta_i_xo, vector beta_i_xa, vector beta_i_xr, vector beta_i_po, vector beta_i_pa, vector beta_i_pr, vector beta_i_to, vector beta_i_ta, vector beta_i_tr) {
    int len = end-start+1;
	vector[] X;
    vector[] TT;
    vector[] R;
	...
	for(j in 1:len){
	  ...
	  X[j] = beta_i_xo[Eid]*xs[k]+beta_i_xa[Eid]*xd[k]+beta_xr[Eid]*xr[k];
    ...
    norm_temp += normal_lpdf(k_slice|theta,0.1);
    return norm_temp;
  }
}
data{
...
model{
  ...
  beta_i_xo ~ normal(0,sd_i);
  beta_i_po ~ normal(0,sd_i);
  beta_i_to ~ normal(0,sd_i);
  beta_i_xa ~ normal(0,sd_i);
  beta_i_xr ~ normal(0,sd_i);
  beta_i_pa ~ normal(0,sd_i);
  beta_i_pr ~ normal(0,sd_i);
  beta_i_ta ~ normal(0,sd_i);
  beta_i_tr ~ normal(0,sd_i);
  
  //likelihood
  target += reduce_sum(partial_sum,k,grainsize,Exp,xs,xd,xr,ps,pd,pr,ts,td,tr,beta_i_xo,beta_i_xa,beta_i_xr,beta_i_po,beta_i_pa,beta_i_pr,beta_i_to,beta_i_ta,beta_i_tr);
}
