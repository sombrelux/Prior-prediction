...
generated quantities{
  ...
  for(k in 1:nPart){
    alpha[k] = trunc_normal_rng(0.1,0.5,10^(-5),positive_infinity());
    beta[k] = beta_rng(2,4);
    gamma[k] = beta_rng(3,1);
    R[k] = trunc_normal_rng(1,1,0,positive_infinity());
    S[k] = trunc_normal_rng(8,2,0,positive_infinity());
