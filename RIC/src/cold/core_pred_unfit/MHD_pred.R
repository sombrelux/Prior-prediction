...
sig_df <- read.csv('./RIC/src/4_core_pred_unfit/MHD_sig.csv',header = T)
parameters <- 'ypred'
...
for(i in 1:4){
  data<-list(
    ...
    mu_a = mu_post[1], mu_c = 0,#peak at 0
    mu_loghd = mu_post[3], mu_loghr = mu_post[4], 
	  mu_sr = mu_post[5], mu_sd = mu_post[6], 
    mu_s = mu_post[7],
	  sig_a = sig_df[1,i], sig_c = sig_df[2,i], 
    sig_loghd = sig_df[3,i], sig_loghr = sig_df[4,i],
	  sig_sr = sig_df[5,i], sig_sd = sig_df[6,i], 
    sig_s = sig_df[7,i])
    samples <- stan(file='./RIC/src/4_core_pred_unfit/prior_MHD_normal.stan',
                data=data,
                pars=parameters,
                iter = 20000,
                warmup = 0,
	...
	saveRDS(samples,paste0('./RIC/output/results/core_pred_unfit/prior_MHD_normal_',i,'.rds'))
}
...
i=1
samples <- readRDS(paste0('./RIC/output/results/core_pred_unfit/prior_MHD_normal_',i,'.rds'))
...
write_csv(hdi_mhd,paste0('./RIC/output/results/core_pred_unfit/hdi_MHD_normal_',i,'.csv'))
...
write_csv(hdi_eff_mhd,
            paste0('./RIC/output/results/core_pred_unfit/hdi_MHD_eff_',i,'.csv'))
