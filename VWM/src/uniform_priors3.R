...
for(i in 15:17){#exp1_dt$nPart){
  ...
  D <- rbind(D, data$D)
  m <- rbind(m, data$m)
  samples <- stan(file='./VWM/src/prior_3_g.stan',
                  data=data,
                  pars=parameters,
                  iter=2000, 
                  chains=4, 
  ...
  saveRDS(samples,paste0("./VWM/output/results/Prior_3/Prediction/subj_",i,".rds"))
  Sys.sleep(30)
}
saveRDS(Setsize,"./VWM/output/results/Prior_3/setsize.rds")
saveRDS(D,"./VWM/output/results/Prior_3/D.rds")
saveRDS(m,"./VWM/output/results/Prior_3/m.rds")

dir <- getwd()
setwd('./VWM/output/results/Prior_3/Prediction')
subj_files <- list.files()
prior_3 <- NULL
for(i in 1:length(subj_files)){
  samples <- readRDS(subj_files[i])
  ypred <- t(extract(samples)$ypred) #800, 4000
  prior_3 <- rbind(prior_3,ypred)
}
dim(prior_3)
setwd(dir)
saveRDS(prior_3,"./VWM/output/results/Prior_3/prior_pred_pool.rds")

# 2 Plots ----------------------
rm(list=ls())
m <- readRDS("./VWM/output/results/Prior_3/m.rds")
D <- readRDS("./VWM/output/results/Prior_3/D.rds")
setsize <- readRDS("./VWM/output/results/Prior_3/setsize.rds")
ypred <- readRDS('./VWM/output/results/Prior_3/prior_pred_pool.rds')
...
ytarg <- m[,1]
error_prior_3 <- apply(ypred_rad,2,
                       function(u) wrap(u-ytarg))
dim(error_prior_3)
range(error_prior_3) #-pi,pi

error_prior_3 <- data.frame(error_prior_3,
                            setsize=setsize)

mean_err <- abs(error_prior_3)%>%
  dplyr::group_by(setsize)%>%
  summarise_at(vars(X1:X4000),mean)%>%
  pivot_longer(!setsize,names_to='sim',
               values_to = 'mean')
saveRDS(mean_err,"./VWM/output/results/Prior_3/mean_err.rds")

resp_err <- error_prior_3%>%
  pivot_longer(!setsize,names_to = 'sim',
               values_to = 'error')
saveRDS(resp_err,"./VWM/output/results/Prior_3/resp_err.rds")
...
ggsave("./VWM/output/fig/Prior_3/Mean absolute response error.svg")
...
p
ggsave("./VWM/output/fig/Prior_3/response error.svg")

### fig3 right =======
## differences between resp & non-targ
trial <- setsize>1
diff_prior_3 <- apply(ypred_rad[trial,ind],2,
                      function(u) wrap(u-m[trial,-1]))
length(diff_prior_3)

diff_prior_3 <- lapply(diff_prior_3, 
...
diff_prior_3 <- abind(diff_prior_3,along = 2)
dim(diff_prior_3)
...
p3 <- ggplot(diff_prior,aes(x=error,group=sim))+
  ...
  labs(x='Deviation from non-target',
       y='Density')
p3
ggsave("./VWM/output/fig/Prior_3/diff.svg")
...
ggsave("./VWM/output/fig/Prior_3/err_dist.svg")

# prior ------
hdi_im<-hdi(as.data.frame(t(error_prior_3)),ci=0.8)
hdi_im<-hdi_im%>%
  add_column(
    setsize = data$Setsize,
    mean = rowMeans(error_prior_3))%>%
