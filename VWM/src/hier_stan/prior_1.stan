...
generated quantities{
  ...
  //informative priors
  mu_a = trunc_normal_rng(0,0.2,0,positive_infinity());
  mu_b = trunc_normal_rng(0,0.2,0,positive_infinity());
  mu_r = trunc_normal_rng(0,0.1,0,1);
  mu_sloc = trunc_normal_rng(5,3,0,positive_infinity());
  mu_scol = trunc_normal_rng(5,3,0,positive_infinity());
  mu_kappa = trunc_normal_rng(7,3,0,positive_infinity());
  mu_delta = trunc_normal_rng(20,10,0,positive_infinity());
  mu_w = beta_rng(3,2);
  
  for(i in 1:nPart){
    a[i] = trunc_normal_rng(mu_a,0.2,0,positive_infinity());
    b[i] = trunc_normal_rng(mu_b,0.2,0,positive_infinity());
    r[i] = trunc_normal_rng(mu_r,0.1,0,1);
    sloc[i] = trunc_normal_rng(mu_sloc,10,0,positive_infinity());
    scol[i] = trunc_normal_rng(mu_scol,10,0,positive_infinity());
    kappa[i] = trunc_normal_rng(mu_kappa,10,0,positive_infinity());
    delta[i] = trunc_normal_rng(mu_delta,10,0,positive_infinity());
  ...
  for(j in 1:nTrial){
    ...
    for(k in 1:N){
      ...
      Aa[j,k] = sum(vm_temp);
      Af[j,k] = von_mises_pdf_1d(X[k],m[j,1],kappaf[id]);
    }
	  Ax[j] = Ac[j]+p*Af[j]+(a[id]*Aa[j]+b[id]*Ab)*q[id];
