...
mae_err_vdb <- readRDS("./VWM/output/results/data_prior/mae_err_vdBerg.rds")
mae_err_dp <- data.frame(cond=c('Both','Color','Location'),
                         DP_low = rep(min(mae_err_vdb$mae)-sd(mae_err_vdb$mae),3),
                         DP_high = rep(min(max(mae_err_vdb$mae)+sd(mae_err_vdb$mae),pi/2),3))
## observed ======
...
# loc has similar mae for color & orient
# color & loc same good cue for orient
mae_devnt_vdb <- readRDS("./VWM/output/results/data_prior/mae_devnt_vdBerg.rds")
mae_devnt_dp <- data.frame(cond=c('Both','Color','Location'),
                           DP_low = rep(min(mae_devnt_vdb$mae)-sd(mae_devnt_vdb$mae),3),
                           DP_high = rep(min(max(mae_devnt_vdb$mae)+sd(mae_devnt_vdb$mae),pi/2),3))
## observed ======
...
mae_dloc_obs <- data.frame(
  cbind(err_dist_1$mae,
        err_dist_2$mae,err_dist_3$mae),
  cond=c('1','2','3'))%>%
  rename('Dloc=1/13'=X1,'Dloc=2/13'=X2,'Dloc>2/13'=X3)%>%
  pivot_longer(!cond,names_to = 'dist',
               values_to = 'obs')%>%
  mutate(cond = dplyr::recode(cond,"1" = "Both",
                              "2" = "Color",
                              "3" = "Location"))
...
mae_dcol_obs <- data.frame(
  cbind(err_dist_1$mae,
        err_dist_2$mae,err_dist_3$mae),
  cond=c('1','2','3'))%>%
  rename('Dcol=1/9'=X1,'Dcol=2/9'=X2,'Dcol>2/9'=X3)%>%
  pivot_longer(!cond,names_to = 'dist',
               values_to = 'obs')%>%
  mutate(cond = dplyr::recode(cond,"1" = "Both",
                              "2" = "Color",
...
mae_dloc <- merge(mae_dloc_ci,mae_dloc_dp,
                  by=c('cond','dist'))%>%
  merge(.,mae_dloc_obs,by=c('cond','dist'))
...
mae_dcol <- merge(mae_dcol_ci,mae_dcol_dp,
                  by=c('cond','dist'))%>%
  merge(.,mae_dcol_obs,by=c('cond','dist'))
